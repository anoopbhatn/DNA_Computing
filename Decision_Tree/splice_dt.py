...
actual_attr =range(len(dataset[0])-1)
...
def makeDecision(d,check):
	...
	if type(d) is not dict:
		...
		#inp=raw_input().strip()
		inp=check[actual_attr.index(attribute)]
	
		d=d[attribute][inp]
		
		result=makeDecision(d,check)
		return result
...
for row in dataset:
	result=makeDecision(d,row)
	if result==row[-1]:
		correct_count+=1
	else:
		wrong_count+=1

print 'The accuracy of Decision Tree is :',(float(correct_count)/(correct_count+wrong_count))*100,'%'
